...
void AudioWrapper::noteOn(double frequency) {
    std::lock_guard<std::mutex> lock(mMutex);

    for (auto& osc : harmonicOscillators) {
        if (osc.getFrequency() == frequency) return;
    }
    ...
    harmonicOscillators.push_back(harmonicOscillator);
}

void AudioWrapper::noteOff(double frequency) {
    std::lock_guard<std::mutex> lock(mMutex);

    harmonicOscillators.erase(std::remove_if(harmonicOscillators.begin(), harmonicOscillators.end(),
            [frequency](HarmonicOscillator& harmonicOscillator) {
                return (harmonicOscillator.getFrequency() == frequency);
            }));
}
